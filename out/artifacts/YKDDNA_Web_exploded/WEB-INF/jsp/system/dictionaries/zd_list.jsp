...
<div class="row-fluid">
	...
	<!-- 检索  -->
	<form action="dictionaries/list.do" method="post" name="userForm" id="userForm">
	...
	<table>
		<tr>
			...
			<c:if test="${pd.PARENT_ID != '0'}">
				...
				<td style="vertical-align:top;"><a href="dictionaries/list.do?PARENT_ID=0" class="btn btn-mini btn-purple" title="查看">顶级<i class="icon-arrow-right  icon-on-right"></i></a></td>
				<c:forEach items="${varsList}" var="var" varStatus="vsd">
				<td style="vertical-align:top;"><a href="dictionaries/list.do?PARENT_ID=${var.ZD_ID }" class="btn btn-mini btn-purple" title="查看">${var.NAME }<i class="icon-arrow-right  icon-on-right"></i></a></td>
				</c:forEach>
				</c:when>
				<c:otherwise>
				</c:otherwise>
	...
	<table id="table_report" class="table table-striped table-bordered table-hover">
		...
		<tr>
			...
			<th class='center'  style="width: 100px;">名称</th>
			<th class='center'  style="width: 100px;">编码</th>
			<th class='center'  style="width: 50px;">级别</th>
		...
		<c:choose>
			<c:when test="${not empty varList}">
				...
				<td class='center'><a href="dictionaries/list.do?PARENT_ID=${var.ZD_ID }" title="查看下级"><i class="icon-arrow-right  icon-on-right"></i>&nbsp;${var.NAME }</a></td>
				<td class='center'>${var.P_BM }</td>
				<td class='center' style="width:35px;"><b class="green">${var.JB }</b></td>
				<td class='center' style="width: 68px;">
				<a title="编辑" onclick="edit('${var.ZD_ID }')" >编辑</a>
					<a title="删除"  onclick="del('${var.ZD_ID }')">删除</a></td>
				</tr>
				</c:forEach>
			</c:when>
			<c:otherwise>
		...
		<table style="width:100%;">
			<tr>
				...
				<c:if test="${pd.PARENT_ID != '0'}">
					<td style="vertical-align:top;" class="left"><a class="btn btn-small btn-info" onclick="location.href='dictionaries/list.do?PARENT_ID=${pdp.PARENT_ID }';">返回</a></td>
				</c:if>
