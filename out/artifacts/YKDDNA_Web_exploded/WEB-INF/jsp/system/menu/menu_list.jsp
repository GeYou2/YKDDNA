			...
			$.get(url,function(data){
				if(data.length>0){
					...
					$.each(data,function(i){
						...
						html += "</td>";
						html += "<td class='center'>"+this.MENU_URL+"</td>";
						html += "<td class='center'>"+this.MENU_ORDER+"</td>";
						html += "<td class='center'><a title='编辑' onclick='editmenu(\""+this.MENU_ID+"\")'><i class='icon-edit'></i></a> <a  title='删除' onclick='delmenu(\""+this.MENU_ID+"\",false)'><i class='icon-trash'></i></a></td>";
						html += "</tr>";
	...
	<table id="table_report" class="table table-striped table-bordered table-hover">
		...
		<c:choose>
			<c:when test="${not empty menuList}">
				...
				<a  title="展开" onclick="openClose('${menu.MENU_ID }',this,${vs.index })" >展开</a>
				<a  title="图标" onclick="editTb('${menu.MENU_ID }')" >图标</a>
				<a  title="编辑" onclick="editmenu('${menu.MENU_ID }')" >编辑</a>
				<a  title="删除"  onclick="delmenu('${menu.MENU_ID }',true)">删除</a></td>
				</tr>
				</c:forEach>
			</c:when>
			<c:otherwise>
